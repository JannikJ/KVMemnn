...
class KVMMModel(nn.Module):
    def __init__(self, pad_length=20, batch_size=100, embedding_size=200, n_chars=20, vocab_size=1000, n_labels=20,
        ...
        self.dialogue_output = nn.Linear(400, vocab_size)  # 1954  # 978

        self.input_embed_keyvalue = nn.Embedding(self.vocab_size, self.embedding_size, 431)
        self.keyvalue_dense1 = nn.Sequential(nn.Linear(kb_size, 20), nn.Tanh())  # 431
    ...
    def forward(self, input_dialogue, input_keyvalues):
        ...
        dense1 = self.dense1_dialogue(
            encoder[0])  # apply tanh on after applying linear transformation on encoder output
        dense2 = self.dense2_dialogue(
            decoder[0])  # apply tanh on after applying linear transformation on decoder output
        dense3 = self.dense3_dialogue(torch.add(dense1,
                                                dense2))  # combine the output from dens1 and dense2, then applying linear trnsformation and tanh funtion.

        attention = F.softmax(dense3, dim=2)  # Apply attention function of the dense3. #Implementation of equation3
        n_hidden = torch.mul(attention, encoder[
            0])  # Weighted summerize the result with attention output and sencode output. #Implementation of equation 4
        output = self.dialogue_output(torch.cat((encoder[0], n_hidden),
                                                dim=2))  # Linear transformation on the output of cascating encoder output and n_hidden #Implementation of equation 5
        ...
        input_embed2 = reshape(input_embed2, self.batch_size, kb_size, self.embedding_size, self.pad_length)  # 431
        n_dense1 = self.keyvalue_dense1(
            input_embed2)  # apply tanh on after applying linear transformation on input_embed2
        ...
        n_dense3 = self.keyvalue_dense3(torch.cat((n_dense1, n_dense2),
